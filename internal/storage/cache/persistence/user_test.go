...
import (
	...
	mock_psql "github.com/martinitorquato/rest-api/mocks/psql"
	"github.com/martinitorquato/rest-api/platform/postgres"
)
...
func Test_userPersistence_Create(t *testing.T) {
	sqlxDB, mocked, db := mock_psql.Connection()
	defer db.Close()
	type args struct {
		ctx  context.Context
		user *model.User
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			got, err := up.Create(tt.args.ctx, tt.args.user)
